let app = new Vue({
      ...
      accounts: [
          {
              username: "admin123",
              password: "admin123",
              fullname: "Phan Xuân A",
          },
          {
              username: "admin234",
              password: "admin123",
              fullname: "Phan Xuân B",
          },
          {
            username: "admin345",
            password: "admin123",
            fullname: "Phan Xuân C",
          },
      ]
    },
    ...
    methods: {
        ...
        if(this.form.username.length < 6 || this.form.username.includes(' ')){
         this.error.username ="Username nhỏ hơn 6 kí tự và không có khoảng trắng";
          _error = false;
        }
        if(this.form.password.length < 6 || this.form.password.includes(' ')){

          this.error.password ="Password nhỏ hơn 6 kí tự và không có khoảng trắng";
          _error = false;
        }
        if(!this.form.policy){
          this.error.policy ="Vui lòng check";
          _error = false;
        }
        ...
        checklogin(){
            ...
            if(item.username === this.form.username && item.password === this.form.password){
            this.success = true;
